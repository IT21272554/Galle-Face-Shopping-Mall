...
const ItemBox = ({ item }) => {
    ...
    const handleNavigateSimilar = (e)=>{
        e.stopPropagation();
        navigate(`/similar/${item.name}`)
    }
    return (
        <Link to={`/item/${item._id}`} className='relative px-0 min-w-40 py-0 bg-cyan-200 rounded-lg hover:scale-105' >
            ...
            <div className='px-4 py-2 flex flex-col text-clip'>
                ...
                {
                    item.description && <span className='text-xs text-nowrap'>{item?.description?.length > 30 ? item.description.substring(0, 30) : item.description}</span>
                }
                ...
                {!item?.inStock && <button className='text-xs px-1 py-1 bg-amber-500 hover:bg-amber-200' onClick={handleNavigateSimilar}>Find Similar Products</button>}
